...
const openWeatherToken = "f624d6f601ac99d9813fbf8c0fe6aed2";

// 날짜 포맷 변경 함수
const getYmd10 = (timestamp) => {
...
const fetchWeather = async (lat, lon) => {
  ...
  currTemp.value = Math.round(data.main.temp * 10) / 10;
  weatherType.value = data.weather[0].description;
  currTime.value = getYmd10(data.dt);
...
onMounted(() => {
  if (props.item.latitude && props.item.longitude) {
    ...
    fetchWeather(props.item.latitude, props.item.longitude);
  }
});
</script>
    ...
    >
      ...
      >
        <div class="flex items-center gap-3">
          ...
          />
          <div class="-mt-px flex flex-col w-2/4">
            <p class="block text-sm font-medium leading-normal text-blue-gray-900 antialiased">
              {{ item.title }}
            </p>
            <p class="block text-xs font-normal text-gray-700 antialiased">@{{ item.addr1 }}</p>
          </div>
          <!-- <div class="w-1/4"></div> -->
          <div class="bg-white rounded-lg shadow-md p-4 w-2/4 max-w-sm mx-auto">
            <div class="flex items-center justify-between mb-4">
              <div class="text-4xl font-bold text-gray-800">{{ currTemp }}°C</div>
              <div class="flex items-center space-x-2">
                <img :src="iconUrl" alt="weather icon" class="h-10 w-10" v-if="iconUrl" />
                <span class="text-gray-600 text-sm">{{ weatherType }}</span>
              </div>
            </div>
      ...
      >
        <template v-if="item.img1">
          <img :alt="item.title" :src="item.img1" class="w-full h-auto max-h-[50vh] object-cover" />
        </template>
        <template v-else>
          <div class="flex items-center justify-center h-[50vh] text-gray-500">
            <h1 class="block text-5xl font-semibold leading-tight tracking-normal antialiased">
              이미지를 준비중입니다 😢
            </h1>
          </div>
        </template>
      ...
      <div class="flex flex-row justify-end mb-5">
        <a
          :href="
            ...
            item.title.replace(/[^\wㄱ-ㅎ가-힣ㅏ-ㅣ]|_/g, '').replace(/\s+/g, '')
          "
          target="_blank"
          class="pt-1.5 max-w-[31px] mr-3"
          @click.stop
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
